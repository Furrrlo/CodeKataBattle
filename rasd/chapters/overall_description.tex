...
for (let pos of final_positions_student) {
	...
	\item \textbf{Students receive a notification about the new team battle and personal students score when a new push to the forked GitHub repository is performed}\\
	      ...
	      Marco logs in, go to the details of the battle and view the new battle and tournament score assigned to him automatically by the platform.\\
...
The forked repo contains build automation scripts and some test cases \comment{??} uploaded by the educator, so that students can try running their code.
...
\begin{enumerate}[label=\textbf{D\arabic*}:,ref=D\arabic*,leftmargin=1.3cm]
	...
	\labelleditem{The school has an existing authentication system that can be used by the system}
